...
def service_create_systemd
  template "#{node['docker']['systemd_system_dir']}/#{service_name}.socket" do
  ...
  template "#{node['docker']['systemd_system_dir']}/#{service_name}.service" do
...
def service_remove_systemd
  ...
  %w(service socket).each do |f|
    file "#{node['docker']['systemd_system_dir']}/#{service_name}.#{f}" do
      action :delete
    end
  end
end
